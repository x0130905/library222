...
这是一份新手向说明。照着做，你可以把这个 PDF 资料库部署到 Netlify，并通过网页后台 `/admin/` 上传 PDF、修改分类和编辑资料信息。
...
- 管理员可以邀请用户、设置角色。
- 管理员可以进入 `/admin/` 后台。
- 管理员可以上传 PDF、修改分类、标题、简介、时间、大小、标签。
...
1. 把 PDF 上传到 GitHub 的 `public/pdfs/`。
2. 把资料信息写入 GitHub 的 `data/pdfs.json`。
